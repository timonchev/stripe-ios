...
extension FinancialConnectionsWebFlowViewController {
    ...
    private func startAuthenticationSession(
        ...
        guard authSessionManager == nil else {
            return
        }
                ...
                case .success(.success(let returnUrl)):
                    if manifest.isProductInstantDebits {
                        if let paymentMethodId = Self.extractValue(from: returnUrl, key: "payment_method_id") {
                            let instantDebitsSession = InstantDebitsSession(paymentMethodId: paymentMethodId)
                            self.notifyDelegateOfSuccess(result: .instantDebits(instantDebitsSession))
                        } else {
    ...
    private func notifyDelegateOfSuccess(result: HostControllerCompletedResult) {
